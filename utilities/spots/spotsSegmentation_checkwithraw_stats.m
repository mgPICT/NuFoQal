function [spots, signal, spheresout] = spotsSegmentation_checkwithraw_stats(imori, imDec, GFP, DEC, signal, dims, spotsize, resXY, resZ, gaussfit, fileName, sigQual, minSigTh)
%rrr
% Spots
% 1: X
% 2: Y
...
% 7: Position (distance to membrane)
    ...
    if isnan(minSigTh)
        ...
        [Jraw] = HoG_blobs3D(GFPd, options3d);
        Jraw = (Jraw - min(Jraw(:)))./range(Jraw(:));

        Jall = imreconstruct(J, Jraw);
        % Jall = imreconstruct(transformed, Jraw);
        Jall = (Jall - min(Jall(:)))./range(Jall(:));

        r2dareas = [max(floor(spotsize(1)), 3), ceil(pi*3*spotsize(end))];
        regions = cell([dims(3), 1]);
        cc = cell([dims(3), 1]);
        for ii = 1:dims(3)
            [regions{ii},cc{ii}] = detectMSERFeatures(Jall(:,:,ii), 'ThresholdDelta', .1, 'RegionAreaRange', r2dareas, 'MaxAreaVariation', 2); %.5
        end
        MJ = false(dims);
        for iz = 1:dims(3)
            Mz = false(dims(1:2));
        ...
        MJc = imerode(MJ, [0 1 0; 0 1 0; 0 1 0]);
        MJc = bwareaopen(MJc, 9);
        MJc = imdilate(MJc, [0 1 0; 0 1 0; 0 1 0]);

        % Check if local maxima
        maxloc = imregionalmax(imhmax(Jall, 1.5*std(Jall(:))), 26);
        MJ = imreconstruct(maxloc, MJc);
        ...
        cc = cell([dims(3), 1]);
        for ii = 1:dims(3)
            [regions{ii},cc{ii}] = detectMSERFeatures(J(:,:,ii), 'ThresholdDelta', .1, 'RegionAreaRange', r2dareas, 'MaxAreaVariation', 2); %.5
        end
        MJ = false(dims);
        for iz = 1:dims(3)
            Mz = false(dims(1:2));
