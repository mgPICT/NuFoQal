...
for i = 1:length(sigmas),
    ...
    % tra = LambdaAbs1+LambdaAbs2+LambdaAbs3;
    % det = LambdaAbs1.*LambdaAbs2.*LambdaAbs3;
    if (options.BlackWhite)
        LambdaAbs1(Lambda1 < 0) = 0;
        LambdaAbs2(Lambda2 < 0) = 0;
        LambdaAbs3(Lambda3 < 0) = 0;

        tra = Lambda1 + Lambda2 + Lambda3;
        det = Lambda1.*Lambda1.*Lambda1;
    else
        LambdaAbs1(Lambda1 > 0) = 0;
        LambdaAbs2(Lambda2 > 0) = 0;
        LambdaAbs3(Lambda3 > 0) = 0;
        ...
        det = imcomplement(Lambda1.*Lambda1.*Lambda1);
    end
    ...
    % Free memory
    clear LambdaAbs1 LambdaAbs2 LambdaAb3
    ...
    A = 2*options.HoGAlpha^2; B = 2*options.HoGBeta^2;  C = 2*options.HoGC^2;

    expTra = (1-exp(-(tra./A)));
    expDet = (1-exp(-(det./B)));
    expS  = (1-exp(-S.^2./C));

    % tradet = imreconstruct(expDet, expTra);
...
%     clear S A B C Ra Rb
    ...
    %Compute blobness function
    Voxel_data = expTra.* expDet .*expS;
    % Voxel_data = tradet .*expS;
    % Free memory
    clear expTra expDet;
    
    if(options.BlackWhite)
        Voxel_data(Lambda1 < 0)=0; Voxel_data(Lambda2 < 0)=0; Voxel_data(Lambda3 < 0)=0;
    else
        Voxel_data(Lambda1 > 0)=0; Voxel_data(Lambda2 > 0)=0; Voxel_data(Lambda3 > 0)=0;
    end
